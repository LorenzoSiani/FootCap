# FootCap Architecture V1.2

**Status:** Approved clarification before Phase 0.  
**Supersedes:** FOOTCAP_ARCHITECTURE_V1.1.md as implementation baseline; prior versions remain historical records.
...
V1 output is home/draw/away probability forming a valid distribution. publication_state is PUBLISHED or SUPERSEDED.
...
A replacement creates a new Prediction, records supersedes_prediction_id on that new Prediction, changes the previous official Prediction to SUPERSEDED, and publishes the new one. Provenance remains intact. Supersession is reconstructed through IDs, never timestamps alone.

### 7.9 Reproducibility invariant

Completed feature, training, prediction, and backtest records; Training Feature Sets; Prediction Feature Snapshots; artifacts; and predictions are immutable. Every prediction traces to exact Dataset, Training Feature Set, Training Run, artifact, Prediction Feature Snapshot/hash, code, dependencies, configuration, and temporal_classification.
...
Each fold records training/prediction cutoff(s), temporal_classification, training context, linked feature set/run/artifact/snapshots/predictions, outcomes, metrics, and baseline comparison. Historical benchmarks are Event-Time Safe unless availability is demonstrated.
...
GET /api/v1/matches/:id/predictions returns only evaluation_context = PRODUCTION. It may include PUBLISHED and SUPERSEDED predictions to show official evolution; each exposes publication_state and supersedes_prediction_id where present. It never returns BACKTEST or EXPERIMENT.
...
5. Completed feature/model/prediction/backtest records are immutable.
